...
export interface VechileCategoryActionResponse<T> {
  success: boolean;
  status_code: number;
  data: T;
}
...
export class VechileCategoryAction {
  private static async handleResponse<T>(
    response: Response
  ): Promise<VechileCategoryActionResponse<T>> {
  ...
  ): VechileCategoryActionResponse<T> {
  ...
  static async StoreNewVechileCategory(
    payload: NewVechileCategoryParamater
  ): Promise<VechileCategoryActionResponse<NewVechileCategoryParamater>> {
    try {
      const action = await api(`/api/admin/vechile-category`, {
        method: "POST",
      ...
      return this.handleResponse<NewVechileCategoryParamater>(action);
    } catch (error: any) {
      console.error(error);
      return this.handleFetchError<NewVechileCategoryParamater>(
        error.response || error
      );
    }
  }

  static async UpdateVechileCategory(
    payload: UpdateVechileCategoryParamater,
    vechileCategoryUuid: string,
  ): Promise<VechileCategoryActionResponse<UpdateVechileCategoryParamater>> {
    try {
      const action = await api(`/api/admin/vechile-category/${vechileCategoryUuid}`, {
        method: "PUT",
        body: JSON.stringify(payload),
      });
      ...
      return this.handleResponse<UpdateVechileCategoryParamater>(action);
    } catch (error: any) {
      console.error(error);
      return this.handleFetchError<UpdateVechileCategoryParamater>(
        error.response || error
      );
    }
  }
