...
export interface PolicyActionServerResponse<T> {
  success: boolean;
  status_code: number;
  data: T;
}
...
export class PolicyActionServer {
  private static async handleResponse<T>(
    response: Response
  ): Promise<PolicyActionServerResponse<T>> {
  ...
  static async GetPolicyTnCFromAdmin(): Promise<
    PolicyActionServerResponse<GetPolicyResponse>
  > {
    try {
      const action = await apiServer("/api/admin/policy/term-and-conditions", {
        method: "GET",
      });
      ...
      return this.handleResponse<GetPolicyResponse>(action);
    } catch (error: any) {
      console.error(error);
      return this.handleFetchError<GetPolicyResponse>(error.response || error);
    }
  }

  static async GetPolicyTnCFromCustomer(): Promise<
    PolicyActionServerResponse<GetPolicyResponse>
  > {
    try {
      const action = await apiServer(
        "/api/customer/policy/term-and-conditions",
        {
          method: "GET",
        }
      );
