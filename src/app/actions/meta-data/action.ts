...
export interface MetaDataActionResponse<T> {
  success: boolean;
  status_code: number;
  data: T;
}
...
export class MetaDataAction {
  private static async handleResponse<T>(
    response: Response
  ): Promise<MetaDataActionResponse<T>> {
  ...
  static async GetActivityMetaData(): Promise<
    MetaDataActionResponse<Array<ActivityMetaDataResponse>>
  > {
    try {
      const action = await api("/api/admin/meta-data-list", {
        method: "GET",
      });
  ...
  static async EditMetaData(
    metaDataUuid: string,
    payload: EditMetaDataParamater
  ): Promise<MetaDataActionResponse<string>> {
    try {
      const action = await api(`/api/admin/meta-data/edit/${metaDataUuid}`, {
        method: "PUT",
