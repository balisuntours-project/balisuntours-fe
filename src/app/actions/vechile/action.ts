...
export interface VechileActionResponse<T> {
  success: boolean;
  status_code: number;
  data: T;
}
...
export class VechileAction {
  private static async handleResponse<T>(
    response: Response
  ): Promise<VechileActionResponse<T>> {
  ...
  static async StoreNewVechile(
    payload: FormData
  ): Promise<VechileActionResponse<Omit<GetSingleVechileResponse, "vechile_main_photo_file">>> {
    try {
      const action = await api(`/api/admin/vechile`, {
        method: "POST",
        body: payload,
      }, false);
      ...
      return this.handleResponse<Omit<GetSingleVechileResponse, "vechile_main_photo_file">>(action);
    } catch (error: any) {
      console.error(error);
      return this.handleFetchError<Omit<GetSingleVechileResponse, "vechile_main_photo_file">>(
        error.response || error
      );
    }
  }

  static async UpdateVechile(
    payload: FormData,
    uuid: string
  ): Promise<VechileActionResponse<Omit<GetSingleVechileResponse, "vechile_category" | "uuid">>> {
    try {
      const action = await api(`/api/admin/vechile/${uuid}`, {
        method: "POST",
        body: payload,
      }, false);
      ...
      return this.handleResponse<Omit<GetSingleVechileResponse, "vechile_category" | "uuid">>(action);
    } catch (error: any) {
      console.error(error);
      return this.handleFetchError<Omit<GetSingleVechileResponse, "vechile_category" | "uuid">>(
        error.response || error
      );
    }
  }
