...
import { EditAdditionalServiceSchema } from "../validation/edit.validation";

export function EditAdditinalServiceForm({currentData, serviceUuid} : {currentData: AdditionalServiceItemResponse, serviceUuid: string}) {
  ...
  const editAdditionalServiceForm = useForm<
    z.infer<typeof EditAdditionalServiceSchema>
  >({
    resolver: zodResolver(EditAdditionalServiceSchema),
    defaultValues: {
      name: currentData.name,
      price: currentData.price,
      min_qty: currentData.min_qty,
      max_qty: currentData.max_qty,
    },
  });

  const setIsLoading = useLoaderStore((state) => state.setIsLoading);

  const handleEditAdditionalService = async (
    values: z.infer<typeof EditAdditionalServiceSchema>
  ) => {
    ...
    setIsLoading(true);
    const result = await AdditionalServiceAction.UpdateAdditionalService(payload, serviceUuid);
    ...
    toast({
      description: `Additional service updated!`,
      variant: "success",
    });
  ...
  return (
    <>
      <TextLoader title="Hold a second" text="Updating service..." />
      <div className="mx-auto w-1/2 p-6">
        <ActivityTitleCard title="Edit additional service" />
        <Form {...editAdditionalServiceForm}>
          <form
            onSubmit={editAdditionalServiceForm.handleSubmit(
