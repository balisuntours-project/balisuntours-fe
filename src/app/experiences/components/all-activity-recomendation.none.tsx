...
export function AllActivityRecomendationWhenFilterNone({
  children,
}: {
  children: React.ReactNode;
}) {
  const recomendedActivities = useAllActivityStore(
    (state) => state.recomendedActivities
  );
      ...
      <>
        {recomendedActivities.length > 0 ? (
          <BestCategoryActivityDrawer
            title={`Have no clue yet?`}
            description="Try these legit activities!"
            activities={recomendedActivities}
          >
            <span className="text-blue-500 underline cursor-pointer">
              or whe have these recomendation for you!
            </span>
          </BestCategoryActivityDrawer>
        ) : (
          ""
  ...
  return (
    <>
      <div className="flex flex-col col-span-4 items-center justify-center p-4 text-center mx-auto">
        <TentTree
          strokeWidth={2.75}
          className="h-14 w-14 md:h-9 md:w-9 text-gray-400 mb-4"
        />
        ...
        <div className="w-[220px] max-w-full mx-auto mt-3">{children}</div>
      </div>
    </>
  );
}
